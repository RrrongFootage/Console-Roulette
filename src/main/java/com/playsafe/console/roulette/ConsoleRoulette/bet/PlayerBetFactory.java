...
import com.playsafe.console.roulette.ConsoleRoulette.bet.Bet.Type;
import com.playsafe.console.roulette.ConsoleRoulette.player.Player;
import com.playsafe.console.roulette.ConsoleRoulette.player.PlayersStorage;
...
class PlayerBetFactory {
	...
	public Optional<PlayerBet> createPlayerBet(String input) {
		return Optional.ofNullable(input).flatMap(val -> {
			...
			if (playerBetMatcher.matches()) {
				...
				return Optional.of(name).flatMap(this::findPlayer).map(player -> new PlayerBet(player, createBet(playerBetMatcher)));
			} else {
	...
	private Optional<Player> findPlayer(String name) {
		return playersStorage.getRegisteredPlayers().stream().filter(p -> Objects.equals(p.getName(), name)).findAny();
	}

	private Bet createBet(Matcher playerBetMatcher) {
		String betGroup = playerBetMatcher.group(2);
		String betAmountGroup =
				playerBetMatcher.group(3);
